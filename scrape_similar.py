...
def download_model(dwnld_url, output_dir, get_thumb=True):
    ...
    try:
        ...
        print 'something wrong with url %s, bailing'%dwnld_url
...
#parser.add_argument('query_mid', nargs=1, help='mid of query models')
...
#with open('curated0.json') as f:
with open('models/existing.json') as f:
    curated = json.load(f)

while True:
    # choose random seed from curated
    #mid = random.choice(existing_model_mids)
    mid = random.choice(curated)
    query_url = similar_query_url(mid)
    print tc.colored('querying similar to', 'green'),  mid
    download_query(query_url, existing_model_mids, output_dir)
